...
import dns from 'dns';
import { JsonRpcProvider, Contract, formatUnits, type BigNumberish, BaseContract } from 'ethers';
import fs from 'fs';
...
const UNI_GQL = `https://gateway-arbitrum.network.thegraph.com/api/${THE_GRAPH_API_KEY}/subgraphs/id/5zvR82QoaXYFyDEKLZ9t6v9adgnptxYpKpSbxtgVENFV`;
...
const UNI_Q = gql`
    query ($owner: Bytes!) {
        positions(where: { owner: $owner }, first: 20, orderBy: id, orderDirection: desc) {
            id
            pool {
                feeTier
                token0 {
...
const main = async (): Promise<void> => {
    ...
    const uniRes = await request<UniResponse>(UNI_GQL, UNI_Q, { owner: ADDR });
    console.log(
        'Uniswap data fetched, positions found:',
        uniRes.positions?.length || 0,
        JSON.stringify(uniRes, null, 2),
    );

    const positionsUni = uniRes.positions || [];
    ...
    console.log(
        `Searching for a Uniswap position in a ${TARGET_TOKEN_A}/${TARGET_TOKEN_B} pool with a ${TARGET_FEE_TIER} fee tier.`,
    );

    const pos = positionsUni.find((p) => {
        if (!p.pool || !p.pool.token0 || !p.pool.token1) {
            return false;
        }

        const feeTier = p.pool.feeTier;
        const s0 = p.pool.token0.symbol.toUpperCase();
        const s1 = p.pool.token1.symbol.toUpperCase();
